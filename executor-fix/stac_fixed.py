...
def create_stac_item(href: str) -> Item:
    ...
    with xr.open_dataset(href) as dataset:
        crs = dataset.rio.crs
        logger.info(f"Read CRS from dataset: {crs}")

        # If CRS is None or invalid, try to get from attributes
        if crs is None:
            if 'crs' in dataset.attrs:
                crs_str = dataset.attrs['crs']
                logger.info(f"Trying to parse CRS from attrs: {crs_str[:100] if len(crs_str) > 100 else crs_str}...")
                try:
                    crs = CRS.from_user_input(crs_str)
                    logger.info(f"Parsed CRS from attrs: {crs}")
                except Exception as e:
                    logger.warning(f"Could not parse CRS from attrs: {e}")

            # Check for spatial_ref variable (CF convention)
            if crs is None and 'spatial_ref' in dataset.coords:
                try:
                    crs_wkt = dataset.coords['spatial_ref'].attrs.get('crs_wkt') or dataset.coords['spatial_ref'].attrs.get('spatial_ref')
                    if crs_wkt:
                        crs = CRS.from_wkt(crs_wkt)
                        logger.info(f"Parsed CRS from spatial_ref: {crs}")
                except Exception as e:
                    logger.warning(f"Could not parse CRS from spatial_ref: {e}")

        # Default to EPSG:4326 if still no CRS
        if crs is None:
            logger.warning("No CRS found, defaulting to EPSG:4326")
    ...
    # Convert CRS to pyproj CRS if needed
    if hasattr(crs, 'to_wkt'):
        crs_for_reproject = CRS.from_wkt(crs.to_wkt())
    else:
        crs_for_reproject = crs

    logger.info(f"Reprojecting geometry from {crs_for_reproject} to EPSG:4326")

    geometry = stactools.core.projection.reproject_geom(
        crs_for_reproject, "EPSG:4326", proj_geometry, precision=6
    )
    ...
    if epsg:
        ...
        if hasattr(crs, 'to_wkt'):
            projection.wkt2 = crs.to_wkt()
        else:
            projection.wkt2 = str(crs)
